...
def blendImagePair(warped_image, image_2, point):
    ...
    output_image[int(point[1]):int(point[1]) + image_2.shape[0],
                 int(point[0]):int(point[0]) + image_2.shape[1]] = image_2
    ...
    mask_2[int(point[1]):int(point[1])+image_2.shape[0], int(point[0]):int(point[0])+image_2.shape[1]] = 127
    ...
    max_row = 0
    min_row = 100000
    for row in range(mask_overlap.shape[0]):
        for col in range(mask_overlap.shape[1]):
            if(col>max_col):
        ...
        for col in range(output_image.shape[1]):
            if (mask_overlap[row, col, 0] > 0):
                ...
                if (row<=max_row and row>=max_row-row_bot_threshold):
                    ...
                    output_image[row, col] = y_weight_bot*image_2[int(row-point[1]), int(col-point[0])] + (1-y_weight_bot)*output_image[row, col]
    return output_image
    # END OF FUNCTION
...
image_1 = cv2.imread("images/oda/o1.jpg")
image_2 = cv2.imread("images/oda/o2.jpg")
image_3 = cv2.imread("images/oda/o3.jpg")
image_1_gray = cv2.cvtColor(image_1, cv2.COLOR_BGR2GRAY)
image_2_gray = cv2.cvtColor(image_2, cv2.COLOR_BGR2GRAY)
image_3_gray = cv2.cvtColor(image_3, cv2.COLOR_BGR2GRAY)

image_1_kp, image_2_kp, matches12 = findMatchesBetweenImages(image_1_gray, image_2_gray, 100)
homography_12 = findHomography(image_1_kp, image_2_kp, matches12)
result_12 = warpImagePair(image_1, image_2, homography_12)

result_12_gray = cv2.cvtColor(result_12, cv2.COLOR_BGR2GRAY)
result_12_kp, image_3_kp, matches123 = findMatchesBetweenImages(result_12_gray, image_3_gray, 100)
homography_123 = findHomography(result_12_kp, image_3_kp, matches123)
result_123 = warpImagePair(result_12, image_3, homography_123)

# image_2_kp, image_3_kp, matches23 = findMatchesBetweenImages(image_2_gray, image_3_gray, 100)
# homography_23 = findHomography(image_2_kp, image_3_kp, matches23)
# result_23 = warpImagePair(image_2, image_3, homography_23)


cv2.imwrite("images/oda12.jpg", result_12)
cv2.imwrite("images/oda23.jpg", result_23)
cv2.imwrite("images/oda123.jpg", result_123)